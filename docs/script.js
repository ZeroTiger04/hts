/* ══════════════════════════════════════════════
   HIGH-PERFORMANCE REAL-TIME CHART ENGINE
   Optimization: Double Layer + Instant Tick Update
══════════════════════════════════════════════ */
...
const CONFIG = {
    up: '#00c076', down: '#ff4a5a', bg: '#121519',
    grid: '#1e2227', paddingRight: 90, volRatio: 0.15
};
...
async function loadHistory(iv) {
    ...
    const url = `https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=${iv}&limit=160`;
...
function drawMain() {
    ...
    for(let i=1; i<10; i++) { let x = (width/10)*i; ctxMain.moveTo(x,0); ctxMain.lineTo(x,height); }
    ctxMain.stroke();
    ...
    ctxMain.fillText(last.close.toLocaleString(), width - CONFIG.paddingRight + 8, yL + 4);
}
...
function drawUI() {
    ...
    ctxUI.fillStyle = '#fff'; ctxUI.fillText(hP.toLocaleString(undefined, {maximumFractionDigits:2}), width - CONFIG.paddingRight + 8, mouseY + 4);
}
...
function updateUI(d) {
    ...
    elPrice.className = `price ${pct >= 0 ? 'text-up' : 'text-down'}`;
    elChange.innerText = (pct >= 0 ? '+' : '') + pct.toFixed(2) + '%';
    elChange.className = `change ${pct >= 0 ? 'text-up' : 'text-down'}`;
